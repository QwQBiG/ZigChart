...
The core owns validated OHLCV data, MA/EMA/RSI/MACD/Bollinger Bands calculations, viewport state, hit testing, and chart geometry. The browser owns data access, input, scheduling, labels, and drawing. Canvas 2D is a supported architecture choice; hardware acceleration is managed by the browser.
...
- Unified analysis library with Trend, Volume, Oscillators, and Volatility groups; strategy and script execution remain unimplemented.
...
The sample is not an exchange feed. No credentials, purchased data, trading, Python runtime, or backtest engine are included. No claim of parity with or superiority to existing charting products is made.
...
This is a development MVP. Provider reconnects, exchange sessions, data adjustments, historical insertion/deletion and timestamp changes, additional indicator types and multiple instances, arbitrary panes, public plugin APIs, multiple chart windows, advanced touch gestures, and cross-browser coverage require further work. Local settings and drawing documents do not save market data or a complete workspace. Synthetic samples exercise the pipeline; they do not validate a future provider's data quality or entitlements.
